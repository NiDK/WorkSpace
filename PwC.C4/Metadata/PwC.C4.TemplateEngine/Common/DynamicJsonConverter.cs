...
namespace PwC.C4.TemplateEngine.Common
{
    ...
    public class DynamicJsonConverter : DynamicObject
    {
        ...
        {
            using (var reader = JsonReaderWriterFactory.CreateJsonReader(encoding.GetBytes(json),
        ...
        {
            using (
                var reader = JsonReaderWriterFactory.CreateJsonReader(stream, encoding, XmlDictionaryReaderQuotas.Max,
                    _ => { }))
            {
                return ToValue(XElement.Load(reader));
            }
        ...
        private static dynamic ToValue(XElement element)
        {
            ...
            switch (type)
            {
                ...
                case JsonType.array:
                    return new DynamicJsonConverter(element, type);
                case JsonType.@null:
                default:
                    return null;
            }
        ...
        private static IEnumerable<XStreamingElement> CreateXObject(object obj)
        {
            return obj.GetType()
                ...
                .Select(
                    a => new XStreamingElement(a.Name, CreateTypeAttr(GetJsonType(a.Value)), CreateJsonNode(a.Value)));
        }
        ...
        private DynamicJsonConverter(XElement element, JsonType type)
        {
            xml = element;
            jsonType = type;
        }
        ...
        private dynamic DeserializeValue(XElement element, Type elementType)
        {
            ...
            if (value is DynamicJsonConverter)
            {
                value = ((DynamicJsonConverter) value).Deserialize(elementType);
            }
        ...
        public override bool TryConvert(ConvertBinder binder, out object result)
        {
            if (binder.Type == typeof (IEnumerable) || binder.Type == typeof (object[]))
            {
                var ie = (IsArray)
                    ...
                    : xml.Elements()
                        .Select(x => (dynamic) new KeyValuePair<string, object>(x.Name.LocalName, ToValue(x)));
        ...
        private bool TryGet(XElement element, out object result)
        {
            ...
            {
                result = string.Empty;
                return true;//此处修改为true，就不会报异常了，但是不排除将来该处会有bug的产生 by marshall
            }
