...
namespace PwC.C4.Infrastructure.Helper
{
    ...
    {
        public  static int Utf8Probability(byte[] rawtext)
        {
            int score = 0;
            int i, rawtextlen = 0;
        ...
        public static Encoding GetBytesEncoding(byte[] rawtext)
        {
            var p = Utf8Probability(rawtext);
            if (p > 80)
        ...
        public static Encoding GetBytesEncoding(Stream rawtext)
        {
            var bytes = StreamToBytes(rawtext);
            var p = Utf8Probability(bytes);
            if (p > 80)
                return Encoding.GetEncoding(65001);//utf8   
            else
                return Encoding.Default;
    ...
    public class IdentifyEncoding
    {
        ...
        /// <summary>
        /// 从指定的 <see cref="FileInfo"/> 中判断编码类型
        /// </summary>
        /// <param name="testfile">要判断的 <see cref="FileInfo"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        /// <example>
        /// 以下示例演示了如何调用 <see cref="GetEncodingName"/> 方法：
        /// <code>
        ...
        /// </summary>
        /// <param name="rawtext">要判断的 <see cref="FileInfo"/> </param>
        /// <returns>返回编码类型("GB2312", "GBK", "HZ", "Big5", "CNS 11643", "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER")</returns>
        /// <example>
        /// 以下示例演示了如何调用 <see cref="GetEncodingName"/> 方法：
        /// <code>
