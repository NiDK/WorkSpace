...
namespace PwC.C4.Infrastructure.BaseLogger
{
    public class LogWrapper
    {
        ...
        private static void LoadAdoNetAppender()
        {
            try
            {
                ...
                if (hier == null) return;
                var buffCount = 10;
                int.TryParse(AppSettings.Instance.GetNode(ConfigConstValues.LoggerNodeName, ConfigConstValues.LoggerBuffSize).Value,
                    out buffCount);
                var appender = new AdoNetAppender
                {
                    Name = "AdoNetAppender",
                    CommandType = CommandType.Text,
                    BufferSize = buffCount,
                    ...
                    ConnectionString = AppSettings.Instance.GetConntectStringV2("dbconn.InfrastructureConn"),
                    CommandText = @"INSERT INTO [dbo].[Log_Exception]([AppCode],[Type],[Date],[StaffId],[Thread],[Level],[Message],[Exception],[Logger],[Status]) VALUES (@AppCode,@Type,@Date,@StaffId,@Thread,@Level,@Message,@Exception,@Logger,@Status)"
                };

                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@Date", DbType = System.Data.DbType.DateTime, Layout = new log4net.Layout.RawTimeStampLayout() });
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@AppCode", DbType = System.Data.DbType.String, Size = 50, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%property{AppCode}")) });
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@Type", DbType = System.Data.DbType.String, Size = 255, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%property{Type}")) });
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@StaffId", DbType = System.Data.DbType.String, Size = 255, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%property{StaffId}")) });
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@Thread", DbType = System.Data.DbType.String, Size = 50, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%thread")) });
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@Level", DbType = System.Data.DbType.String, Size = 255, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%level")) });
                ...
                appender.AddParameter(new AdoNetAppenderParameter { ParameterName = "@Status", DbType = System.Data.DbType.Int32, Layout = new Layout2RawLayoutAdapter(new PatternLayout("%property{Status}")) });
        ...
        private static void LoadFileAppender()
        {
            ...
            {
                var logPath =
                AppSettings.Instance.GetNode(ConfigConstValues.LoggerNodeName, ConfigConstValues.LoggerFilePath).Value;
                ...
                var txtLogPath = Path.Combine(logPath, appCode + ".log");
